...
16. **AUTONOMOUS MODE**: `/autonomous full|semi|off`. `off`=default, all checkpoints fire. `semi`=scope/pick/plan approved upfront; per-phase WAITs still fire. `full`=only HARD gates surface (TOOL_FAILURE, blocking BUG_FOUND, bug creation, T4 skill load, env dead, security recalibration). When `autonomous≠off`: BEFORE any skill execution, generate a permission manifest (per `agentic-qa-core/references/permission-manifest-template.md`), present it, WAIT for explicit OK. Then execute with gate bypass per `agentic-qa-core/references/autonomous-gates.md`. Auto-resume + auto-archive per `agentic-qa-core/references/session-management.md` §4+§8.
...
| TMS documentation / ROI                                                  | "document tests", "ROI", "automate priority"                                                                                                                                                                 | `/test-documentation`              | `.context/master-test-plan.md`, `docs/qa-standard/traceability-trello-drive.md`                                                                                                                                           | `[TMS_TOOL]`                                                                                                                                                               |
...
| Discovery / inventory                                                    | "what components exist", "list ATCs", "is TC-X automated", "coverage map", "what's tested", "qué está cubierto"                                                                                              | -                                  | `kata-manifest.json`; coverage map + gaps → `bun run tests:map` (reads `.context/trello/`, offline)                                                                                                                       | Read / `bun run tests:map`                                                                                                                                                 |
...
| Trello operation                                                         | "Trello card", "move card", "add checklist"                                                                                                                                                                  | `trello` CLI                       | `.agents/project.yaml` → `issue_tracker.trello_*`; `docs/qa-standard/traceability-trello-drive.md`                                                                                                                        | CLI                                                                                                                                                                        |
...
- `bun run tests:map`: coverage map — renders the synced `.context/trello/` cache (list → card → AC spine) as one HTML page (`.context/reports/test-map.html`; `--json` for the gap summary). Disk-only, no Jira calls; `bun run trello:sync` first if stale.
...
| `trello`                    | "Trello card", "move card", "add checklist", "add label", "tablero", "tarjeta", "mover tarjeta"                                                                 | Trello board/card/list/label/checklist CLI skill (this repo's `[ISSUE_TRACKER_TOOL]`, replacing Jira/Atlassian). Owns WHEN/WHAT (auth via `TRELLO_API_KEY`/`TRELLO_TOKEN`, checklist item = one AC, slug `{LABEL}-{trello-number}`); the `trello` CLI is the executor, the Google Drive matrix is the traceability spine.                                                                                                                                                                                                                                                                                                                         |
...
| ---------------------- | ------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------- |
| `[ISSUE_TRACKER_TOOL]` | Trello (board / list / card)                                                                                       | `trello` CLI (Scale-Flow/trello-cli)                                                                                                                                            | Trello REST API via curl (`TRELLO_API_KEY` + `TRELLO_TOKEN`)                           |
| `[TMS_TOOL]`           | Test management                                                                                                    | `none` — no TMS integration in this project; tests live in Trello card checklists + a Google Drive matrix (see `docs/qa-standard/traceability-trello-drive.md`)                 | —                                                                                      |
...
> **Reads-vs-writes carve-out**: `[ISSUE_TRACKER_TOOL]` resolves to the `trello` CLI for WRITE / move / comment / label / checklist operations. DETAILED CONTENT reads (card descriptions, checklists, comments) also route through `trello cards get` / `trello checklists list` / `trello comments list` — the CLI returns structured JSON on every command. Traceability feature↔test lives in the Drive matrix, not in a coverage panel (see `docs/qa-standard/traceability-trello-drive.md`).
...
- Before any `[ISSUE_TRACKER_TOOL] ...` → the `trello` CLI (`TRELLO_API_KEY` + `TRELLO_TOKEN` in `.env`)
- Before any `[TMS_TOOL] ...` → `none` — no TMS integration (see the traceability standard)
...
**TMS modality** (resolved by `test-documentation/SKILL.md` §Phase 0):

| Modality            | `[TMS_TOOL]` resolves to                                        | TMS entities                                                            |
| ------------------- | --------------------------------------------------------------- | ----------------------------------------------------------------------- |
| A: none (this repo) | not resolvable → tests live in Trello checklists + Drive matrix | AC checklist items on Trello cards; feature↔test matrix in Google Drive |

Skills using `[TMS_TOOL]` MUST include a parallel pseudocode branch for the `none` modality (labeled "Modality none").
...
| `trello`                                      | `/trello` (in-repo) — board/card/list/label/checklist grammar, auth, slug + checklist=AC conventions; also see `docs/qa-standard/traceability-trello-drive.md`                        |
...
**INSTANCE-IDENTITY ANCHOR — REMOVED (Trello migration)**: this project no longer uses Atlassian. The former `issue_tracker.atlassian_url` anchor and its `cli/lib/atlassian-instance.ts` resolver were removed 2026-09-18. Issue-tracker identity now lives in `.agents/project.yaml` → `issue_tracker.trello_board_id` / `trello_board_url`; credentials are `TRELLO_API_KEY` / `TRELLO_TOKEN` in `.env`. No `{{jira.*}}` variable resolves at runtime anymore (`config.tms.provider` is `none`).
...
## 9. LOCAL CONTEXT (Trello)

> **TRELLO REMAP**: this project runs Trello + Google Drive, not Jira/Xray. The `.context/PBI/` Jira-cache doctrine below is **historical** — it is replaced by `.context/trello/`, a gitignored cache of the Trello board owned by `scripts/sync-trello.ts` (`bun run trello:sync [--board] [--card <id>] [--include-comments]`), plus the committed `.agents/trello-board.json` catalog (`bun run trello:sync-board`, validated by `bun run trello:board:check`). Same three-tier `[SYNC]/[COMMIT]/[LOCAL]` model and gitignore-ladder idea, with Trello primitives (board/list/card/checklist/label) instead of Jira issues. The prose below is retained until the `{{jira.*}}` references are fully rewritten; treat every `bun run jira:*` / `context:hydrate` / `.context/PBI/` mention in it as the Trello equivalent above.
...
### Accepted divergence — retired

The `ProtectPublic` ruleset (id `16809531`) lived on the boilerplate's original org repo. This fork (`nelgoez/talently-qa-novamarket`) has **no host ruleset**, so the former `main.direct_push_to_protected` accepted divergence was removed from `git_strategy.policy.accepted_divergences` (2026-09-18). `bun run git:policy verify` now reports `DRIFT (0)` and "Declared policy matches the host"; `direct_push_to_protected: allowed` is how work lands here (standing authorization — Critical Rule #5).
